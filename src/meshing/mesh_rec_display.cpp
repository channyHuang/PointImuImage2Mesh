...
struct Region_triangles_shader
{
    ...
    void unparse_triangle_set_to_vector( const Triangle_set &tri_angle_set ) {
        ...
        m_triangle_pt_color.resize( tri_angle_set.size() * 3 );
        ...
        for ( Triangle_set::iterator it = tri_angle_set.begin(); it != tri_angle_set.end(); it++ ) {
            ...
            m_triangle_pt_color[count] = g_map_rgb_pts_mesh.m_rgb_pts_vec[ ( *it )->m_tri_pts_id[ 0 ] ]->get_rgb();
            m_triangle_pt_vec[ count + 1 ] = pt_b.cast< float >();
            m_triangle_pt_color[count + 1] = g_map_rgb_pts_mesh.m_rgb_pts_vec[ ( *it )->m_tri_pts_id[ 1 ] ]->get_rgb();
            m_triangle_pt_vec[ count + 2 ] = pt_c.cast< float >();
            m_triangle_pt_color[count + 2] = g_map_rgb_pts_mesh.m_rgb_pts_vec[ ( *it )->m_tri_pts_id[ 2 ] ]->get_rgb();
            count = count + 3;
        }
    }
...
void display_current_LiDAR_pts( int current_frame_idx, double pts_size, vec_4f color ) {
    ...
    g_LiDAR_point_shader.set_pointcloud( g_eigen_vec_vec[ current_frame_idx ].first, vec_3( color[0], color[1], color[2] ) );
...
// 更新生成网格m_triangle_set_vector到渲染队列
