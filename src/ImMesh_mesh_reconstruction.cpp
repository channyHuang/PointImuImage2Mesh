...
void incremental_mesh_reconstruction( pcl::PointCloud< pcl::PointXYZI >::Ptr frame_pts, Eigen::Quaterniond pose_q, Eigen::Vector3d pose_t, int frame_idx ) {
    ...
    // 对当前帧点云剔除过于邻近的点后得到 g_map_rgb_pts_mesh.m_voxels_recent_visited 查找邻近点、三角化后增加新三角面或删除旧三角面，最后仍旧保存到g_eigen_vec_vec中
    int                 append_point_step = 1; //std::max( ( int ) 1, ( int ) std::round( frame_pts->points.size() / appending_pts_frame ) );
...
// 网格重建线程池，每次从g_rec_mesh_data_package_list取重建点云数据放入线程池中进行重建
